			...
			<dd style="line-height: 26px;">
				...
				<c:if test="${orderInfo.order.paystate>=1}">
					...
					<c:if test="${orderInfo.order.paystate==1}">
						...
						<a href="${ pageContext.request.contextPath }/order/deliever?id=${orderInfo.order.id}"> </a>
					</c:if>	
					<c:if test="${orderInfo.order.paystate==2}">
						<font color="orange">已发货</font>
						<a href="${ pageContext.request.contextPath }/order/received?id=${orderInfo.order.id}">用户确认收货</a>
					</c:if>	
